<?php
session_start();

// Initialize cart if not exists
if (!isset($_SESSION['cart'])) {
    $_SESSION['cart'] = [];
}

// Handle quantity updates
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_quantity'])) {
    // Create a temporary array to store updated quantities
    $updated_quantities = [];
    foreach ($_POST['quantity'] as $encoded_name => $qty) {
        $name = urldecode($encoded_name);
        $updated_quantities[$name] = max(1, (int)$qty);
    }
    
    // Update the cart with new quantities while preserving all items
    foreach ($_SESSION['cart'] as &$item) {
        if (isset($updated_quantities[$item['name']])) {
            $item['quantity'] = $updated_quantities[$item['name']];
        }
    }
    unset($item); // Break the reference
}

// Handle item removal
if (isset($_GET['remove'])) {
    $name = urldecode($_GET['remove']);
    $_SESSION['cart'] = array_filter($_SESSION['cart'], function($item) use ($name) {
        return $item['name'] !== $name;
    });
}

// Calculate subtotal
$subtotal = array_reduce($_SESSION['cart'], function($total, $item) {
    return $total + ($item['price'] * $item['quantity']);
}, 0);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Shopping Cart</title>
  <script src="https://cdn.tailwindcss.com"></script>
  <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css" rel="stylesheet"/>
  <style>
    @import url("https://fonts.googleapis.com/css2?family=Inter:wght@400;700&display=swap");
    body {
      font-family: "Inter", sans-serif;
    }
  </style>
</head>
<body class="bg-[#e6ebf1] min-h-screen">
  <header class="flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
    <div class="flex items-center space-x-3">
      <img src="https://storage.googleapis.com/a1aa/image/17d88d51-4e71-4f95-650b-573a2b82c415.jpg" alt="HardwareHub logo" class="w-10 h-10 rounded-full"/>
      <span class="font-bold text-sm text-[#1a1a1a] select-none">HardwareHub</span>
    </div>
    <nav class="hidden md:flex space-x-10 font-bold text-sm text-[#1a1a1a]">
      <a href="home.php" class="hover:underline">Home</a>
      <a href="products.php" class="hover:underline">Products</a>
      <a href="about.php" class="hover:underline">About us</a>
      <a href="#" class="hover:underline">Contacts</a>
    </nav>
    <a href="cart.php" class="text-[#f97316] text-xl md:text-2xl relative">
      <i class="fas fa-shopping-cart"></i>
      <?php if (count($_SESSION['cart']) > 0): ?>
        <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
          <?php echo array_reduce($_SESSION['cart'], function($carry, $item) { return $carry + $item['quantity']; }, 0); ?>
        </span>
      <?php endif; ?>
    </a>
  </header>

  <main class="max-w-5xl mx-auto px-6 pb-12">
    <h1 class="font-extrabold text-[#1a1a1a] text-lg mb-6 select-none">Shopping Cart</h1>

    <?php if (empty($_SESSION['cart'])): ?>
      <div class="bg-white rounded-lg p-6 shadow-sm text-center">
        <p class="text-sm text-[#1a1a1a]">Your cart is empty</p>
        <a href="products.php" class="inline-block mt-4 bg-[#f97316] text-white text-xs font-bold px-6 py-2 rounded hover:bg-[#e25816] transition-colors">
          Continue Shopping
        </a>
      </div>
    <?php else: ?>
      <form method="post" action="cart.php">
        <section class="bg-white rounded-lg p-6 shadow-sm border border-transparent border-b-[#d1d5db] border-b-[1px]">
          <div class="grid grid-cols-12 gap-4 border-b border-[#d1d5db] pb-3 mb-3 text-xs font-bold text-[#1a1a1a]">
            <div class="col-span-6">Products</div>
            <div class="col-span-2 text-center">Price</div>
            <div class="col-span-2 text-center">Quantity</div>
            <div class="col-span-2 text-right">Total</div>
          </div>

          <?php foreach ($_SESSION['cart'] as $item): 
            $item_total = $item['price'] * $item['quantity'];
          ?>
          <div class="grid grid-cols-12 gap-4 items-center border-b border-[#d1d5db] py-3 text-[10px] font-semibold text-[#1a1a1a]">
            <div class="col-span-6 flex items-center space-x-4">
              <img src="<?= htmlspecialchars($item['image']) ?>" alt="<?= htmlspecialchars($item['name']) ?>" class="w-14 h-14 rounded-md bg-[#e5e7eb]"/>
              <div>
                <span class="select-none"><?= htmlspecialchars($item['name']) ?></span>
                <a href="?remove=<?= urlencode($item['name']) ?>" class="text-red-500 text-xs block">Remove</a>
              </div>
            </div>
            <div class="col-span-2 text-center select-none">$<?= number_format($item['price'], 2) ?></div>
            <div class="col-span-2 flex justify-center items-center space-x-1">
              <button type="button" onclick="updateQuantity('<?= urlencode($item['name']) ?>', -1)" class="border border-[#d1d5db] w-6 h-6 text-xs font-semibold text-[#1a1a1a] rounded-sm">−</button>
              <input type="number" name="quantity[<?= urlencode($item['name']) ?>]" value="<?= $item['quantity'] ?>" min="1" class="w-6 h-6 text-center border border-[#d1d5db] rounded-sm">
              <button type="button" onclick="updateQuantity('<?= urlencode($item['name']) ?>', 1)" class="border border-[#d1d5db] w-6 h-6 text-xs font-semibold text-[#1a1a1a] rounded-sm">+</button>
            </div>
            <div class="col-span-2 text-right select-none">$<?= number_format($item_total, 2) ?></div>
          </div>
          <?php endforeach; ?>

          <div class="flex justify-end items-center mt-6 border-t border-[#d1d5db] pt-4 text-[10px] font-bold text-[#1a1a1a]">
            <span class="mr-6 select-none">Subtotal</span>
            <span class="select-none w-20 text-right">$<?= number_format($subtotal, 2) ?></span>
          </div>

          <div class="flex justify-between mt-4">
            <a href="products.php" class="bg-gray-200 text-gray-800 text-xs font-bold px-6 py-2 rounded hover:bg-gray-300 transition-colors">
              Continue Shopping
            </a>
            <button type="submit" name="update_quantity" class="bg-[#0052cc] text-white text-xs font-bold px-6 py-2 rounded hover:bg-[#003d99] transition-colors">
              Update Cart
            </button>
            <a href="checkout.php" class="bg-[#f97316] text-white text-xs font-bold px-6 py-2 rounded hover:bg-[#e25816] transition-colors">
              Checkout
            </a>
          </div>
        </section>
      </form>
    <?php endif; ?>
  </main>

  <script>
    function updateQuantity(name, change) {
      const input = document.querySelector(`input[name="quantity[${name}]"]`);
      let value = parseInt(input.value) + change;
      if (value < 1) value = 1;
      input.value = value;
    }
  </script>
</body>
</html>